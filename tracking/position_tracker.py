...
class PositionTracker:
    def __init__(self, max_distance=80, max_missing_frames=80):
        self.tracks = {}
    ...
    def update(self, detections, frame_id, ts):
        ...
        for bbox in detections:
            ...
            if best_token and best_dist < self.max_distance:
                self.tracks[best_token]['centroid'] = centroid
                self.tracks[best_token]['last_frame'] = frame_id
                self.tracks[best_token]['last_ts'] = ts
    ...
    def flush_all(self):
        remaining = []
        for token, track in self.tracks.items():
            remaining.append((token, track['entry_ts'], track['last_ts']))
        return remaining
